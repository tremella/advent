...
function setup() {
  ...
  snowflakeCanvas = createGraphics(200, 200);
  ...
  unfoldButton = createButton('Unfold Snowflake');
...
function draw() {
  ...
  if (!isUnfolded) {
    ...
    if (currentCut) {
      //update end position
      currentCut.endX = mouseX - width/2;
      currentCut.endY = mouseY - height/2;
...
function mousePressed() {
  if (!isUnfolded && mouseX > 0 && mouseX < width && mouseY > 0 && mouseY < height) {
    let startX = mouseX - width/2;
    let startY = mouseY -height/2;
...
function drawFoldedSnowflake() {
  ...
  beginShape()
  for (let i = 0; i < flakePoints.length; i++){
    vertex(flakePoints[i].x, flakePoints[i].y)
  }
  endShape(CLOSE);
  //draw cuts
  for (let cut of cutLines) {
